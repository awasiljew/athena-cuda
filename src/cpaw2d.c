 ...
 * The iterface fields are located at the left grid cell face normal */
    ...
    for (j=js; j<=je+1; j++) {
      for (i=is; i<=ie+1; i++) {
        ...
        pGrid->B1i[j][i] = -(A3(x1,x2+dx2) - A3(x1,x2))/dx2;
        pGrid->B2i[j][i] =  (A3(x1+dx1,x2) - A3(x1,x2))/dx1;
        pGrid->B3i[j][i] = b_perp*cs;
      }
    }
...
/* Now initialize the cell centered quantities */
    ...
    for (j=js; j<=je; j++) {
      for (i=is; i<=ie; i++) {
        ...
        pGrid->U[j][i].d  = Soln[j][i].d;
        pGrid->U[j][i].M1 = Soln[j][i].M1;
        pGrid->U[j][i].M2 = Soln[j][i].M2;
        pGrid->U[j][i].M3 = Soln[j][i].M3;

        Soln[j][i].B1c = 0.5*(pGrid->B1i[j][i] + pGrid->B1i[j][i+1]);
        Soln[j][i].B2c = 0.5*(pGrid->B2i[j][i] + pGrid->B2i[j+1][i]);
        Soln[j][i].B3c = b_perp*cs;
        pGrid->U[j][i].B1c = Soln[j][i].B1c;
        pGrid->U[j][i].B2c = Soln[j][i].B2c;
        pGrid->U[j][i].B3c = Soln[j][i].B3c;

        Soln[j][i].E = pres/Gamma_1 + 0.5*(SQR(pGrid->U[j][i].B1c) +
                 SQR(pGrid->U[j][i].B2c) + SQR(pGrid->U[j][i].B3c) )
          + 0.5*(SQR(pGrid->U[j][i].M1) + SQR(pGrid->U[j][i].M2) +
                 SQR(pGrid->U[j][i].M3) )/den;
        pGrid->U[j][i].E = Soln[j][i].E;
      }
  }
...
/* compute L1 error in each variable, and rms total error */
  ...
  for (j=js; j<=je; j++) {
    for (i=is; i<=ie; i++) {
      error.d   += fabs(pGrid->U[j][i].d   - Soln[j][i].d  );
      error.M1  += fabs(pGrid->U[j][i].M1  - Soln[j][i].M1 );
      error.M2  += fabs(pGrid->U[j][i].M2  - Soln[j][i].M2 );
      error.M3  += fabs(pGrid->U[j][i].M3  - Soln[j][i].M3 );
      error.B1c += fabs(pGrid->U[j][i].B1c - Soln[j][i].B1c);
      error.B2c += fabs(pGrid->U[j][i].B2c - Soln[j][i].B2c);
      error.B3c += fabs(pGrid->U[j][i].B3c - Soln[j][i].B3c);
      error.E   += fabs(pGrid->U[j][i].E   - Soln[j][i].E  );
    }
  }
