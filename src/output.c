...
/* notice the way how the ndim is increased while filling dim[] */
    ...
    ath_pout(1,"DEBUG %d  ndim=%d  Nx=[%d %d %d]\n",
            new_out.n, new_out.ndim, new_out.Nx1,new_out.Nx2, 0/*,new_out.Nx3*/);
...
/* DEBUG */
    ath_pout(0,"OUTPUT: %d %d %s %s [%g : %g], nx1: %d, nx2: %d\n",
             new_out.n, new_out.ndim, new_out.out_fmt,
             new_out.out, new_out.dmin, new_out.dmax, new_out.Nx1, new_out.Nx2);
...
void data_output(Grid *pGrid/*, Domain *pD*/, Grid_gpu *pGpuGrid, const int flag)
{
  int n;
  int dump_flag[MAXOUT_DEFAULT+1];
  char block[80];
  /* Flag indicates whether we have to copy memory from device to host to perform data output */
  int copy_flag = 0;
 ...
 * set dump flag to input argument, check whether time for output */
  for (n=0; n<out_count; n++) {
    ...
    if (pGrid->time >= OutArray[n].t) {
      ...
      dump_flag[n] = 1;
      /* At least one data dump must be performed - set the copy flag to copy memory from device to host */
      copy_flag = 1;
    }
  }
  ...
  if(copy_flag) {
	  copy_to_host_mem(pGrid, pGpuGrid);
	  printf("Copied\n");
  }
...
/* handle TWO_DIM simulations */
  ...
  // if (pgrid->Nx3 == 1) {
    ...
    for (j=0; j<Nx2; j++) {
      for (i=0; i<Nx1; i++) {
	data[j][i] = (*pout->expr)(pgrid,i+il,j+jl);
      }
    }
    return data;
...
Real expr_ME(const Grid *pG, const int i, const int j/*, const int k*/) {
  return 0.5*(pG->U[j][i].B1c*pG->U[j][i].B1c + 
	      pG->U[j][i].B2c*pG->U[j][i].B2c + 
	      pG->U[j][i].B3c*pG->U[j][i].B3c);
}
//#endif
...
Real expr_V1(const Grid *pG, const int i, const int j/*, const int k*/) {
  return pG->U[j][i].M1/pG->U[j][i].d;
}
Real expr_V2(const Grid *pG, const int i, const int j/*, const int k*/) {
  return pG->U[j][i].M2/pG->U[j][i].d;
}
Real expr_V3(const Grid *pG, const int i, const int j/*, const int k*/) {
  return pG->U[j][i].M3/pG->U[j][i].d;
}
